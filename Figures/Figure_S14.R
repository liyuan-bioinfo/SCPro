...
setwd("")
# Figure_S14c, bar plot showing the protein numbers of quantified and identified proteins 
{
    rm(list=ls())
    ...
    write.csv(plot.data.bar2,
              file = paste0("Figure_12c_ProteinNumber_PG_IdentifiedvsQuantified_Barplot_",Sys.Date(),".csv"),
              fileEncoding = "UTF-8",row.names = F,quote = TRUE)
    
    pdf(paste0("Figure_12c_ProteinNumber_PG_IdentifiedvsQuantified_Barplot_",Sys.Date(),".pdf"),
        width=6,height = 4)
    print(temp.p1)
    dev.off()
}   

# Figure_S14d, PCA plot showing the 8 sub-types of Treg
{
    rm(list=ls())
    obj_list = readRDS(file = "ct_dataset_02.rds")
    temp.df = obj_list$Mini_impute_log2 %>% t() %>% as.data.frame()
    ...
    # save plot
    pdf(file=paste("Figure_14d_PCA_",Sys.Date(),".pdf",sep=""),
        width = 8,height=5)
    print(p1)
    dev.off()
